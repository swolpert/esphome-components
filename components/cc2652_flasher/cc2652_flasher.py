...
@register_action("cc2652_flasher.update_firmware", UpdateFirmwareAction, cv.Schema({cv.Required(CONF_ID): cv.use_id(CC2652Flasher)}), synchronous=True)
...
@register_action("cc2652_flasher.check_update", CheckUpdateAction, cv.Schema({cv.Required(CONF_ID): cv.use_id(CC2652Flasher)}), synchronous=True)
...
@register_action("cc2652_flasher.detect_variant", DetectVariantAction, cv.Schema({cv.Required(CONF_ID): cv.use_id(CC2652Flasher)}), synchronous=True)
