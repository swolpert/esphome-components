...
EntityMatchResult UrlMatch::match_entity(EntityBase *entity) const {
  ...
  if (this->id == object_id) {
    result.matched = true;
    // Log deprecation at debug level (old browser-cached JS produces these on every button press)
#ifdef USE_DEVICES
  ...
  for (button::Button *obj : App.get_buttons()) {
    ...
    if (request->method() == HTTP_GET && entity_match.action_is_empty) {
      ...
      request->send(200, "application/json", data.c_str());
               ...
               (request->method() == HTTP_POST && entity_match.action_is_empty)) {
      // Also handle POST with no action segment — old browser-cached JS sends this format
