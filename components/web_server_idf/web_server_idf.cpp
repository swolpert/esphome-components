...
esp_err_t AsyncWebServer::request_post_handler(httpd_req_t *r) {
  ...
  if (content_type.has_value()) {
    ...
    if (strcasestr_n(content_type_char, content_type_len, "application/x-www-form-urlencoded") != nullptr ||
        strcasestr_n(content_type_char, content_type_len, "text/plain") != nullptr) {
      // Normal form data or plain-text body (e.g. older browser-cached JS) - proceed with regular handling
#ifdef USE_WEBSERVER_OTA
